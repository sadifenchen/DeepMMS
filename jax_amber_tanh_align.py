...
def ener_nonbonded14 (atom_types, nonbonds, sigma, epsilon, chgs):
    ...
    def compute_fn (R, vmax0=jnp.float32(50.0)):
        # Ra and Rb (n_pairs, 3)
...
def get_amber_energy_funs (fname_prmtop, fixed_jatom, kval):
    ...
    def restraint_fun (R, R_0):
        ...
        #dRj = R[fixed_jatom] - R_0
        
        dRj = (R[fixed_jatom] - R[4])

        enr = kval*(jnp.sqrt(jnp.einsum('i,i', dRj, dRj)) - R_0[-1])**2
        
        #enr = kval*jnp.einsum('i,i',dRj,dRj)
